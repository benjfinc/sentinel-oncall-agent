...
def test_full_incident_resolves_and_merges(wired):
    ...
    assert fake.calls.index(T.GITHUB_LIST_CHECK_RUNS) < fake.calls.index(T.GITHUB_MERGE_PR)
    # PR is opened before it is merged.
    assert fake.calls.index(T.GITHUB_CREATE_PR) < fake.calls.index(T.GITHUB_MERGE_PR)
    # Slack ack happens before the merge.
    assert fake.calls.index(T.SLACK_SEND_MESSAGE) < fake.calls.index(T.GITHUB_MERGE_PR)
